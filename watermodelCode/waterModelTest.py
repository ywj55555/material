...
waterImgRootPathList = ['train']

# imgpath
label_data_dir = '/home/cjl/dataset/label/'
# png_path =  'D:/ZF2121133HHX/water/daytime/rgb/'
png_path = 'D:/dataset/lg/needmark/'

# csv2_save_path = log+'class4_allFile500_acc.csv'
# model_dict = {1:'ori_model_hz',2:'ori_model_hz'}
class_nums = 2
model_path = "./IntervalSampleAddFeatureWaterModel/"
LEN = 5
featureTrans = True
if featureTrans:
    inputBands = 21
...
epoch_list = [str(x) for x in [10,20,40,80,150,200,280]]
    ...
    for i in range(cnt):
        ...
        for filename in file_tmp:
            ...
            if os.path.exists(waterImgRootPath + filename + '.img'):
                imgData_tmp = envi_loader(waterImgRootPath, filename, False)
            else:
                for tmpImgPath in waterImgRootPathList:
                    if os.path.exists(waterImgRootPath + tmpImgPath + '/' + filename + '.img'):
                        imgData_tmp = envi_loader(waterImgRootPath + tmpImgPath + '/', filename, False)
                        break
            # t3 = time.time()
        ...
        #     rows, cols = predict_ind.shape
            ...
            for png_i in range(png_num):
                imgGt = cv2.imread(png_path + "rgb" + file_tmp[png_i] + '.tif')
                imgGt = imgGt[5:-5,5:-5]
