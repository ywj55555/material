...
model_list = ['twoBranch2.0', 'onlySpectrumBranch2.0', 'onlySpaceBranch2.0',
              'twoBranch3.0', 'onlySpectrumBranch3.0', 'onlySpaceBranch3.0',]  # 1 2 3

model_save = './model/SkinClothWater18_' + model_list[model_select - 1] + '_' + str(patch_size) + '_' + str(mLearningRate) + '_' + str(mBatchSize) + '_' + str(
...
if __name__ == '__main__':
    ...
    trainDataSpace = trainDataSpace[:, hand_selected_bands, :, :]
    testDataSpace = testDataSpace[:, hand_selected_bands, :, :]
    ...
    if patch_size != 11:
        offset = int((11 - patch_size) / 2)
        trainData = trainData[:, :, offset:-offset, offset:-offset]
        testData = testData[:, :, offset:-offset, offset:-offset]
        trainDataSpace = trainDataSpace[:, :, offset:-offset, offset:-offset]
        testDataSpace = testDataSpace[:, :, offset:-offset, offset:-offset]
    ...
    trainDataset = MyDatasetAddSpac(trainData, trainDataSpace, trainLabel)
    testDataset = MyDatasetAddSpac(testData, testDataSpace, testLabel)
    ...
    if model_select == 1:
        model = spaceSpectrumFusionNet(input_bands_num, class_num, patch_size=patch_size).cuda()
    elif model_select == 2:
        model = spaceSpectrumFusionNet(input_bands_num, class_num, space_used=False).cuda()  # Spectrum
    elif model_select == 3:
        model = spaceSpectrumFusionNet(input_bands_num, class_num, spectrum_used=False).cuda()  # Spectrum
    elif model_select == 4:
        model = spaceSpectrumFusionNet(input_bands_num, class_num, patch_size=patch_size, version=3).cuda()
    elif model_select == 5:
        model = SpectrumNet(input_bands_num, class_num, patch_size=patch_size, version=3).cuda()
    elif model_select == 6:
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.8, patience=8,
                                                           ...
                                                           cooldown=5,  # 触发一次条件后，等待一定epoch再进行检测，避免lr下降过速；
                                                           min_lr=0, eps=1e-08)  # 如果新旧lr之间的差异小与1e-8，则忽略此次更新
    ...
    for epoch in range(mEpochs):
        ...
        for i, data in enumerate(trainLoader, 0):
            img, img_spac, label = data
            img, img_spac, label = Variable(img).float().cuda(), Variable(img_spac).float().cuda(), Variable(
                label).float().cuda()
