...
from utils.paint_rect import dilate_specific_label, erode_all_label,open_specific_label_difsize,open_all_diff_size
...
res_path = './res/'
# mkdir(res_path)
label_path = r'D:\ZY2006224YWJ\material-extraction\graduationCode\alien_material\result\twoBranch3.0_skinClothWater18\extraTest_label\17/'
all_png_path = r'F:\spectralDateset\rgb_rend_5/'
label_path = verify_path(label_path)
all_png_path = verify_path(all_png_path)
...
# 先膨胀皮肤
label_data = dilate_specific_label(label_data, 3, 1)
#
...
# 然后整体开运算
dilate_size = 10
label_data = open_all_diff_size(label_data, 11, 16)
# 求连通区域的数量，然后判断阈值是否进行腐蚀！！
# 逐类别腐蚀
# for specified_label in specified_label_list:  # skin cloth water
#     label_data = open_specific_label_difsize(label_data,8,16,specified_label)

# label_data = open_demo(label_data_tmp,15)
# res_png = png_data
# label_data = open_specific_label_difsize(label_data, 15, 18, 3)
for specified_label in specified_label_list:
    png_data = mask_color_img(png_data,label_data==specified_label,color=[color_list[specified_label-1]])
cv2.imwrite(res_path + label_name[:-4] + '_open2.png',png_data)
