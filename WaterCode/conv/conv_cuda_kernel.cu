...
template <typename scalar_t>
__global__ void conv_backward_grad_in_kernel(
    scalar_t *grad_out,
    scalar_t *weight,
    scalar_t *grad_in,
    const int num_elem,
...
template <typename scalar_t>
__global__ void conv_backward_grad_weight_kernel(
    scalar_t *grad_out,
    scalar_t *input,
    ...
    const int PW, const int PH)
{
    ...
    scalar_t tval = reduceBlock<scalar_t, ReduceAdd<scalar_t, scalar_t>>(
        buf, blockDim.x, grad, ReduceAdd<scalar_t, scalar_t>(), 0);
    ...
    int PW, int PH)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(output.type(), "conv_cuda_forward", ([&] {
                                   conv_forward_kernel<scalar_t><<<num_blocks, NUM_THREADS>>>(
                                       input.data<scalar_t>(),
    ...
    int PW, int PH)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(grad_in.type(), "conv_cuda_backward_grad_in", ([&] {
                                   conv_backward_grad_in_kernel<scalar_t><<<num_blocks, NUM_THREADS>>>(
    ...
    int PW, int PH)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(grad_weight.type(), "conv_cuda_backward_grad_weight", ([&] {
                                   conv_backward_grad_weight_kernel<scalar_t><<<grid, block, block.x * sizeof(scalar_t)>>>(
                                       grad_out.data<scalar_t>(),
                                       input.data<scalar_t>(),
