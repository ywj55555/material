...
class DBDA_network_three_losses_cross_Attention(nn.Module):#No Attention, three losses
    def __init__(self, band, classes):
        ...
        self.conv11 = nn.Conv2d(in_channels=band, out_channels=16, padding=0,
                                kernel_size=1, stride=1)
        # Dense block
        self.batch_norm11 = nn.Sequential(
        ...
        self.conv14 = nn.Conv2d(in_channels=48, out_channels=16, padding=0,
                                kernel_size=1, stride=1)
        self.batch_norm14 = nn.Sequential(
                                    nn.BatchNorm2d(64),
                                    nn.ReLU(inplace=True)
...
# all_branch
        self.conv31 = nn.Conv2d(in_channels=band, out_channels=16,
                                kernel_size=1, stride=1)
        # self.conv21 = nn.Conv2d(in_channels=band, out_channels=1,
        #                         kernel_size=1, stride=1)
        # self.conv21_0 = nn.Conv2d(in_channels=1, out_channels=24, padding=1,
    ...
    def forward(self, X):
        ...
        # print('x1', x1.shape)
        # x1 = torch.mul(x1, x16)
        X = self.attention_spectral(X_norm, X)
...
class CAM_Module_Cross_with_Sobel(nn.Module):
    """ Channel attention module"""
    def __init__(self):
        super(CAM_Module_Cross, self).__init__()
        self.conv_3d_1 = nn.Conv3d(in_channels=1, out_channels=5, padding=(0, 1, 1),
                  kernel_size=(1, 3, 3), stride=(1, 1, 1))
        ...
        self.conv_3d_2 = nn.Sequential(nn.ReLU(inplace=True),
                                       nn.Conv3d(in_channels=10, out_channels=3, padding=(0, 1, 1),
                                   kernel_size=(1, 3, 3), stride=(1, 1, 1))
                                       )
        self.gamma = nn.Parameter(torch.zeros(1))
        self.softmax  = nn.Softmax(dim=-1)
...
class Sobel_cal(nn.Module):
    def __init__(self):
        super(Sobel_cal, self).__init__()
        self.Con3d_1 =
