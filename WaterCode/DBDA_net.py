...
class DBDA_network_LSTM_three_losses(nn.Module):#No Attention, three losses
    def __init__(self, band, classes):
        super(DBDA_network_LSTM_three_losses, self).__init__()
        band_1 = band * 50# * 4
        # spectral branch
        # self.bilstm_model = BiLSTM(input_size=50,hidden_size=50,num_layers=2,output_size=60)
        self.conv11 = nn.Conv2d(in_channels=band_1, out_channels=24, padding=1,
                                kernel_size=3, stride=1)  # 24
        # Dense block
        self.batch_norm11 = nn.Sequential(
            nn.BatchNorm2d(24),  # 动量默认值为0.1
        ...
        self.attention_spectral = Conv_Sobel_Extractor()#CAM_Module(60)#
        self.attention_spatial = PAM_Module(60)
    ...
    def forward(self, X):
        X_extra, X_space  = self.attention_spectral(X)
        X_norm = X_extra#
        # spectral
        x11 = self.conv11(X_norm)
        # print('x11', x11.shape)
        ...
        x16 = self.conv15(x16)
        x1 = x16
        # x1 = self.bilstm_model(X_norm)

        # spatial
        #print('x', X.shape)
        #X: batch, band, H, W
        # X = X /  X.max(dim=2, keepdim=True)[0].max(dim=3, keepdim=True)[0]
        X_spat = X#torch.cat([X, X_space], dim=1)
        x21 = self.conv21(X_spat)
        # x21 = self.conv21_0(x21)
        x22 = self.batch_norm21(x21)
        x22 = self.conv22(x22)
...
class Single_network(nn.Module):#No Attention, three losses
    def __init__(self, band, classes):
        ...
        self.conv12 = nn.Conv2d(in_channels=24, out_channels=48, padding=0,
                                kernel_size=1, stride=1)
        self.batch_norm12 = nn.Sequential(
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True)
        )
        self.conv13 = nn.Conv2d(in_channels=48, out_channels=72, padding=0,
                                kernel_size=1, stride=1)
        self.batch_norm13 = nn.Sequential(
            nn.BatchNorm2d(72),
            nn.ReLU(inplace=True)
        )
        self.conv14 = nn.Conv2d(in_channels=72, out_channels=96, padding=1,
                                kernel_size=3, stride=1)
        self.batch_norm14 = nn.Sequential(
            nn.BatchNorm2d(96),
            nn.ReLU(inplace=True)
        ...
        self.attention_spectral = Conv_3d_Sobel()#Conv_Sobel_Extractor()#
        self.attention_spatial = PAM_Module(60)
    ...
    def forward(self, X):
        x2 = self.Spat_forward(X)
        x1 = self.Spec_forward(x2)
        # model1
        x1 = self.global_pooling(x1)
        output_spec = self.full_connection_spec(x1)
        return output_spec

    def Spat_forward(self, X):
        X_spat = X  # torch.cat([X, X_space], dim=1)
        x21 = self.conv21(X_spat)
        # x21 = self.conv21_0(x21)
        x22 = self.batch_norm21(x21)
        ...
        x24 = self.batch_norm23(x24)
        x24 = self.conv24(x24)
        X_1 = self.attention_spectral(X)
        x25 = torch.cat((X, X_1, x21, x22, x23, x24), dim=1)#

        x2 = x25

        return x2
    def Spec_forward(self,X):
        x11 = self.conv11(X)
        # print('x11', x11.shape)
        x12 = self.batch_norm11(x11)
        x12 = self.conv12(x12)
        ...
        x13 = x12#torch.cat((x11, x12), dim=1)
        # print('x13', x13.shape)
        x13 = self.batch_norm12(x13)
        x13 = self.conv13(x13)
        # print('x13', x13.shape)

        x14 = x13#torch.cat((x11, x12, x13), dim=1)
        x14 = self.batch_norm13(x14)
        x14 = self.conv14(x14)

        x15 = x14#torch.cat((x11, x12, x13, x14), dim=1)
        # print('x15', x15.shape)
...
class Conv_Sobel_Extractor(nn.Module):
    ...
    def forward(self,x):
        ...
        X = torch.cat([x_1,x_2,x_3,x_4],dim=1).view(m_batchsize, -1, height, width)
        x_1 = x_1.view(m_batchsize, -1, height, width)
        return X, x_1
...
#         H = input_seq.shape[3]
#         W = input_seq.shape[4]
#         band = input_seq.shape[2]
#         feature_lenth = input_seq.shape[1]
#         input_seq = input_seq.permute(2,0,3,4,1).view(band,-1,feature_lenth)
...
#         pred = pred[-1, :, :].view(batch, H, W, -1).permute(0,3,1,2)
#         print(pred.shape)
#         return pred